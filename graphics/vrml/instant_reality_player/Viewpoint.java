...
class Viewpoint 
{
	...
	public static void main(String[] args)
	{
		...
		try
		{
            ...
            browser.beginUpdate();
            vrml.eai.Node view = browser.getNode("V0"); 
            dumpViewpoint( view );
            browser.endUpdate();
    ...
    public static void dumpViewpoint( vrml.eai.Node view )
    {
        String type = view.getType(); 
        if(type.compareTo("Viewpoint") != 0) return ; 
        System.out.println("view = " + view + " " + type );
       
        vrml.eai.field.EventInSFVec3f positionIn = (vrml.eai.field.EventInSFVec3f)view.getEventIn("set_position");
        vrml.eai.field.EventOutSFVec3f positionOut = (vrml.eai.field.EventOutSFVec3f)view.getEventOut("position");
        float[] position = positionOut.getValue();

        System.out.println("from position = " + position[0] + "," + position[1] + "," + position[2] ); 
        position[2] = position[2]/2 ; 
        System.out.println("  to position = " + position[0] + "," + position[1] + "," + position[2] ); 
        positionIn.setValue(position);
