...
void analysisOM_1mev_gamma(void) {

    printRes("Resolution of a 1 Mev gamma in AD", genPlot("peGen_all", gh_peGen_all));
    printRes("Resolution of a 1 MeV gamma in target", genPlot("peGen_GdLS", gh_peGen_GdLS));
    genPlot("peGenCap_GdLS_iav",gh_peGenCap_GdLS_iav);
    printEff("1 Mev gammas stopping at iav", gh_peGen_GdLS, gh_peGenCap_GdLS_iav);
    gSystem->Exit(0);
...
TF1* genPlot(TString term, TH1F *gh) {
    ...
    TF1 *gf = new TF1("gf","gaus",60,180);
    gf->SetLineColor(2);

    TCanvas *c = new TCanvas;
    gh->SetAxisRange(0,250);
    gh->SetAxisRange(0,200,"Y");
    //gh->Fit("gaus");
    gh->Fit("gf","R");
    gh->Draw();
