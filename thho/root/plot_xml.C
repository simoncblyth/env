...
TGraphErrors* plot(void){
	...
	//gSystem->Load(XMLMap.C);
	gROOT->ProcessLine(".L XMLMap.C");
	TString path =  Form("%s/G4dyb/data/xml/MaterialProperties.xml",gSystem->Getenv("DYW")) ;
	XMLMap* xm = new XMLMap ;
	xm->Parse( path );
	TMap* m = xm->fMap ;
	
	TCanvas* c1 = new TCanvas("c1","attenuation v.s. wavelength",200,10,700,500);
	c1->SetFillColor(42);
	c1->SetGrid();
	...
	cout << " br1" << endl;
	gr = plotg(xm,"/PMT_Property/QE_Momentum","/PMT_Property/QE",70);
        gr->SetLineColor(1);
	gr->Draw("AL");
        gl->Add(gr);
...
/*	
	TGraphErrors* gr1 = plotg(m,"qe_100",70);
        gr1->SetLineColor(1);
	gr1->Draw("L");
...
TGraphErrors* plotg(void* pmap, TString xpath, TString ypath, Double_t wy){

    XMLMap* xmap = (XMLMap*)pmap ;
    return xmap->MakeGraph( xpath , ypath , 1.0 , wy );     	

	
}	
	/*
TGraphErrors* oldplotg(TString filewl, TString fileabs, Int_t we){
