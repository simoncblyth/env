...
class VolMatcher(Matcher):
    ...
    patns = (
               ( 'SKIP'      , re.compile("(?P<all>logic.*|Lead.*|Worldmuon.*|Door.*|inner_log.*|Lowermuon_log_0|reflectivelayer_log_0|(?P<pos>top|bot)_film_log_([0-1]))$") , { 'color':kOrange } ),
#
               
               	( 'L0'        , re.compile("(?P<all>2m_Plastic_Scintillator_log_[0-7])$")    	, { 'color':kGreen  }  ),  
		( 'L1'        , re.compile("(?P<all>1m_Plastic_Scintillator_log_(1[6-9]|2[0-5]))$")      	, { 'color':kBlue   }  ),
 		( 'L2'        , re.compile("(?P<all>1m_Plastic_Scintillator_log_(3[2-9]|4[0-1]))$")          	, { 'color':kYellow }  ),  
               	( 'L3'        , re.compile("(?P<all>1.5m_Plastic_Scintillator_log_(4[8-9]|5[0-5]))$")     	, { 'color':kPink   }  ),
