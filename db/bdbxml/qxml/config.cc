...
int qxml_popvm(int argc, char **argv, po::variables_map& vm )
{
    try {
		...
		("outxml,o", po::value<string>()->default_value(""), "dbxml uri to write query output, which must be valid xml ")
		("level,l",  po::value<string>()->default_value("INFO"), "logging level ")
		...
		("container.source.source",  po::value<svec>(),   "container source directories or exist servlet urls  ")
