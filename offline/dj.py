...
class %(proxy)s(%(name)s):
    ...
    def __unicode__(self):
        return "%(ffmt)s" %(perc)s  ( %(sfmt)s ) 

## methods on the generated model ... not the proxy 

%(name)s.simflag = lambda self:e('SimFlag',self.simmask )
%(name)s.site    = lambda self:e('Site',   self.sitemask )
%(name)s.__unicode__ = lambda self:"%(ffmt)s" %(perc)s ( %(sfmt)s )
...
"""
    ...
    def __repr__(self):
        ...
        for name, cls in self.dj.items():
            ...
            shorts = [] 
            for f in fields:
                if f.startswith('Pmt'):f = f[3:]
                shorts.append(f)     

            list_display = str([cls._meta.pk.name] + shorts )
