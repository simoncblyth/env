...
\abstract{\input{opticks-blyth-chep2026-v0-abstract.tex}}
\maketitle
%
%
...
The hybrid workflow replaces the Geant4 generation of optical photon secondary tracks in a loop
within the scintillation and Cerenkov processes with the collection of "genstep" parameters
into the genstep array that is uploaded to the GPU to enable an equivalent optical photon generation
to be implemented within CUDA threads. The genstep parameters include the number of photons
and the line segment where to generate them. An Opticks CUDA launch can be regarded to simulate in parallel all
optical photons from a slice of the collected genstep array. Slice ranges are chosen such that all necessary GPU buffers
fit within the available VRAM. Each CUDA thread handles the full simulation of an optical photon from generation through to
termination at absorption or detection for example. Scintillator reemission is also treated as "rebirth" 
of the photon within the same single thread. Each optical photon thread has a unique index used to control
where the {\tt sphoton.h} struct details such as position, time, momentum, direction, 
wavelength and history flags are written into the photon buffer. 
 
Each photon thread requires access to its associated parent genstep parameters. A GPU "seed" buffer
provides this association between photon indices and genstep indices.  This seed buffer
is populated directly on the GPU using an NVIDIA CUDA Thrust based implementation.
The Opticks {\tt iexpand} function uses photon counts from the genstep array to populate 
an array of genstep indices for every photon in the launch. 

CUDA Thrust parallel primitives, including {\tt exclusive\_scan}, {\tt scatter\_if} and {\tt inclusive\_scan}
are used to provide a GPU implementation that avoids CPU loops or the need to upload additional arrays to the GPU.  
...
Events with multiple muons with long paths crossing the scintillator yield the most optical photons.  
...
optical photons collected at PMTs, thus the simulation provide an option to collect and save
...
within the standard JUNO simulation. For CPU simulation this approach is considered 
adequate as the processing time for the merging is a small fraction of the overall
processing time. 
...
\subsection{CUDA Thrust Implementation of GPU hit merging} 

\begin{center}
\begin{table}
\begin{tabular}{ |m{80mm}|m{30mm}|m{30mm}|m{60mm}| } 
 \hline
Opticks Event Array          &  Struct/Type      &  NumPy shape               &  Note      \\ 
\hline\hline                                                                 
   genstep                   & {\tt quad6}       &  (num\_genstep,6,4)        & Required   \\
 \hline                                                                      
   seed                      & {\tt int}         &  (num\_photon)             & Optional   \\
 \hline                                                                      
   photon                    & {\tt sphoton}     &  (num\_photon,4,4)         & Required   \\
 \hline                                                                      
   hit                       & {\tt sphoton}     &  (num\_hit,4,4)            & Optional   \\
 \hline
   hitmerged                 & {\tt sphoton}     &  (num\_hitmerged,4,4)      & Optional   \\
 \hline
   photonlite                & {\tt sphotonlite} &  (num\_photon,4,4)         & Optional   \\ 
 \hline
   hitlite                   & {\tt sphotonlite} &  (num\_photon,4)           & Optional   \\
 \hline
   hitlitemerged             & {\tt sphotonlite} &  (num\_hitmerged,4)        & Optional   \\
 \hline
   record                    & {\tt sphoton}     &  (num\_photon,32,4,4)      & Optional - up to 32 photon steps for debugging \\  
 \hline
   seq                       & {\tt sseq}        &  (num\_photon,4,2)         & Optional - history sequence flags 2x64 bit \\  
\end{tabular}
\caption{\label{tabeventarrays}Principal event arrays with their types. The "lite" arrays use summary 16 byte {\tt sphotonlite} struct. The photon and hit arrays
use 64 byte {\tt sphoton} struct. Only genstep and photon arrays are required.  All arrays can be persisted using NumPy serialization for python debugging.  
\end{table}
\vspace{-8mm}
\end{center}%

Opticks event arrays are summarized in~\ref{tabeventarrays}.  
Only the genstep and photon arrays are mandatory, with population and download of all other arrays optional. 
Typically one of the hit, hitlite, hitmerged OR hitlitemerged arrays is populated and downloaded. 
The {\tt thrust::copy\_if} parallel primitive is used to apply history flag selection to
the photon(photonlite) array to yield the hit(hitlite) subset array and merging is
applied to yield hitmerged(hitlitemerged) arrays. As hit(hitlite) selection and merging are needed 
for both the detailed 64 byte {\tt sphoton} struct and the summary 16 byte {\tt sphotonlite} struct
arrays the implementations are templated. As hit selection and merging are closely related they are
implemented in a single method {\tt SPM::merge\_partial\_select}. The method arguments include 
the selection flag mask and the
timewindow in nanoseconds where zero is special cased to disable merging,
...
is formed such that the photons from every genstep array slice fit within the maximum number of slots.
...
\hline 
  hash(pmtid,timebucket) from hit     & {\tt transform}        \\ 
\hline 
...
\label{intro}
%
Opticks[1-7] enables Geant4~\cite{g4} based simulations 
...
Opticks was presented to the six prior CHEP conferences, with each contribution
