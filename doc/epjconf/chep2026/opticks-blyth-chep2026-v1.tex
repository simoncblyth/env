...
large high-yield detectors such as the 20 kton Jiangmen Underground Neutrino Observatory (JUNO)~\cite{juno}. Although developed
for JUNO, Opticks has been used by other experiments including LZ~\cite{lz}, NEXT~\cite{next} 
and EIC~\cite{eic}. Prior CHEP proceedings [1–7] cover the development history of Opticks.
...
%
%
\section{Scaling Opticks+JUNOSW for Monte Carlo Productions}
%
While Opticks can eliminate the optical photon bottleneck in Geant4 simulations, 
achieving high GPU resource efficiency across production Monte Carlo campaigns remains
challenging. Opticks optical simulation performance doubles
with each new NVIDIA RTX generation due to advances in ray-tracing
hardware making the difficulty of fully utilizing these GPU resources
effectively doubles as well. 
Within a monolithic application the GPU remains idle during
Geant4 and framework initialization and during non-optical simulation.
As simulation is CPU-bound by non-optical physics, overlapping local CPU work with GPU propagation yields minimal gains.
However, adopting a server-client architecture enables aggregating CPU workloads from dozens of worker nodes
onto a centralized GPU server saturates the GPU while resolving the shortage of dedicated GPU hardware.



%Multi-threaded Geant4 accumulating gensteps into a shared thread-safe queue can provide some 



\comment{
What are the main challenges of scaling Monte Carlo productions using Opticks
GPU optical photon simulation to accelerate Geant4 ?  Main ones are access to
enough GPUs, and making good use of available GPUs. Any other challenges ? What
strategies could mitigate ? What about server/client to share GPU resources to
non-GPU nodes ?
}
