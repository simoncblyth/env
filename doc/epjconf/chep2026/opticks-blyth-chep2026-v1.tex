...
using CUDA Thrust parallel primitives (\texttt{sysrap/iexpand.h}). 
...
theoretical $4\times$ structural ratio due to transient workspace allocations
...
As a stress test of multi-launch simulation an artifical event with 8.25 billion optical photons
...
or Shader Execution Reordering (SER). Future work will address this lack in the context
...
Although Opticks achieves an optical photon speedup exceeding 1000x, Amdahl's law dictates that the overall simulation speedup
is bounded by the remaining non-optical processing time in Geant4.
%
