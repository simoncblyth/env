...
single rays and a bounding box program that provides the bounds of each geometry primitive.   
These programs together with a ray payload data structure are combined 
into a ray tracing kernel by a domain optimized Just-In-Time compiler.

The central data structure used by ray tracing is a spatial index known 
as an acceleration structure which sorts a scene’s primitives 
into spatial groups. Heirarchical tree structures are typically used
