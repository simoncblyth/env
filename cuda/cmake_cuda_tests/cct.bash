# === func-gen- : cuda/cmake_cuda_tests/cct fgp cuda/cmake_cuda_tests/cct.bash fgn cct fgh cuda/cmake_cuda_tests
cct-src(){      echo cuda/cmake_cuda_tests/cct.bash ; }
cct-source(){   echo $BASH_SOURCE ; }
...
CMake 3.8 3.9 : New CUDA Capabilities
---------------------------------------
...
cct-dir(){ echo $(local-base)/env/cuda/cmake_cuda_tests ; }
cct-cd(){  cd $(cct-dir); }
cct-get(){
   local dir=$(dirname $(cct-dir)) &&  mkdir -p $dir && cd $dir

   [ ! -d cmake_cuda_tests ] && git clone https://gitlab.kitware.com/robertmaynard/cmake_cuda_tests
}
cct-pdf(){ open ~/opticks_refs/S7438-robert-maynard-build-systems-combining-cuda-and-machine-learning.pdf ; }


cct-name(){ echo cmake_cuda_tests  ; }

cct-edir(){ echo $(dirname $(cct-source)) ; } 
cct-bdir(){ echo /tmp/$USER/env/$(cct-name)/build ; } 
cct-sdir(){ cct-dir ; } 
