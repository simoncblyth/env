#!/usr/bin/env python

import logging, sys, re, os
log = logging.getLogger(__name__)
from env.sqlite.db import DB

class WikiPage(object):
    def __init__(self, db, name):
        self.name = name
        self.tags = map(lambda _:str(_[0]), db("select tag from tags where tagspace=\"wiki\" and name=\"%s\" " % name ))

        rec = db("SELECT version,time,author,text,comment,readonly FROM wiki WHERE name=\"%s\" ORDER BY version DESC LIMIT 1" % name ) 
        version,time,author,text,comment,readonly = rec[0] 

        self.version = version
        self.time = time
        self.author = author
        self.text = text
        self.comment = comment

    def __repr__(self):
        return "%5s : %30s : %10s : %15s : %60s : %s " % ( self.version, self.name, self.author, self.time, ",".join(self.tags), self.comment )

    def __str__(self):
        return "\n\n".join( [repr(self), repr(self.text) ] ) 


class Lines(list):
    def __repr__(self):
        return "<%s %s lines> " % (self.__class__.__name__, len(self))
    def __str__(self):
        return "\n".join([repr(self)] + list(self))
    def indent(self, n):
        return map(lambda _:" "*n + _, list(self)) 
  
class Para(Lines):
    def as_rst(self):
        return "\n".join(self)

class Literal(Lines):
    start = "{{{"
    end = "}}}"

    @classmethod 
    def is_start(cls, line):
        return line.startswith(cls.start)
    @classmethod 
    def is_end(cls, line):
        return line.startswith(cls.end)

    def as_rst(self):
        return "\n".join(["","::", ""] + self.indent(4) )


class Toc(Lines):     
    def as_rst(self):
        return "\n".join(["",".. toctree::", ""] + self.indent(3) + ["",""] )


class Head(Lines):
    ptn = re.compile("^(=*) ([^=]*) (=*)$")

    mkr = ["=", "-", "~" ]

    def as_rst(self):
        return "\n".join(["", self.title, self.mkr[int(self.level)-1] * len(self.title) ])

    def __repr__(self):
        return "<Head %s %s %s lines> " % (self.title, self.level, len(self))

    @classmethod 
    def is_head(cls, line):
        return len(line) > 0 and line[0] == "=" and line[-1] == "=" 

    @classmethod 
    def match(cls, line):
        m = cls.ptn.match(line)
        assert m 
        a, title, b = m.groups()
        assert a == b 
        level = len(a)
        return title, level

    @classmethod
    def from_line(cls, line):
        assert cls.is_head(line)
        title, level = cls.match(line) 
        head = cls(title, level)
        head.append(line)
        return head

    def __init__(self, title, level):
        self.title = title
        self.level = level



class Content(list):
    def __repr__(self):
        return "\n".join(map(repr, list(self)))

    def __str__(self):
        return "\n".join(map(str, list(self)))

    def as_rst(self):
        return "\n".join(map(lambda _:_.as_rst(), list(self)))


class Wiki2RST(object):
    skips = r"""
    [[TracNav
    """.lstrip().rstrip().split()

    def end_para(self):
        if self.cur_para is None:return
        self.content.append(self.cur_para)
        self.cur_para = None
    def end_literal(self):
        if self.cur_literal is None:return
        self.content.append(self.cur_literal)
        self.cur_literal = None

    def __init__(self, text, content):
        self.orig = text
        self.content = content

        log.info("Wiki2RST __init__ " ) 

        lines = filter(lambda _:not(_.startswith(tuple(self.skips))), text.split("\r\n"))
        lines = map(str, lines)

        self.cur_para = None
        self.cur_literal = None

        for line in lines:
            if Head.is_head(line):
                head = Head.from_line(line)
                self.content.append(head) 
                self.end_para()
            elif Literal.is_start(line):
                self.cur_literal = Literal()
                self.end_para()
            elif Literal.is_end(line):
                self.end_literal()
            else:
                if self.cur_literal is not None:
                    self.cur_literal.append(line)
                else:
                    if self.cur_para is None:
                         self.cur_para = Para()
                    pass
                    self.cur_para.append(line) 
                pass 
            pass
        pass
        self.end_para()



class Sphinx(object):
    def __init__(self, base, title):
        self.base = base
        self.title = title
        self.pages = []

    def rstpath(self, name):
        path = os.path.join(self.base, "%s.rst" % name )
        dir_ = os.path.dirname(path)
        if not os.path.isdir(dir_):
            os.makedirs(dir_)
        pass
        return path

    def add(self, page):
        self.pages.append(page)  

    def write_(self, page):
        rstpath = self.rstpath(page.name) 
        log.info("write %s " % rstpath )
        open(rstpath, "w").write(page.as_rst())

    def write(self):
        idx = self.make_index("index", self.title)
        self.write_(idx) 
        for page in self.pages:
            self.write_(page)
        pass

    def make_index(self, name, title):
        idx = Page(name)        
        hdr = Head(title, 1)
        toc = Toc()
        for page in self.pages:
            toc.append(page.name)
        pass

        foot = Head("indices and tables", 1)

        para = Para()
        para.append("")
        para.append("* :ref:`genindex`")
        para.append("* :ref:`modindex`")
        para.append("* :ref:`modindex`")
        para.append("")

        idx.append(hdr) 
        idx.append(toc) 
        idx.append(foot) 
        idx.append(para) 

        return idx

class Page(Content):

    @classmethod
    def from_trac(cls, db, name):
        wikipage = WikiPage(db, name)
        print repr(wikipage)
        page = cls(name)
        conv = Wiki2RST(wikipage.text, page)
        return page 

    def __init__(self, name):
        Content.__init__(self)
        self.name = name


if __name__ == '__main__':
    logging.basicConfig(level=logging.INFO)

    dbpath = sys.argv[1] if len(sys.argv) > 1 else None
    assert dbpath
    name = sys.argv[2] if len(sys.argv) > 2 else None

    base = os.path.dirname(os.path.dirname(os.path.dirname(dbpath)))
    base = os.path.join(base, "wiki2rst" )
     
    title = "workflow tracwiki translated"

    sphinx = Sphinx(base, title)
    log.info("base:%s" % base)
    db = DB(dbpath)

    if name is None:
        names = db("select distinct name from wiki") 
        for name, in names:
            page = Page.from_trac(db, name)
            sphinx.add(page)
        pass
    else:
        page = Page.from_trac(db, name) 
        sphinx.add(page)
    pass
    sphinx.write()




