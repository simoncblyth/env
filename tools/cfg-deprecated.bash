...
cfg-parse-nightmare(){
    ...
    m=${#ini[*]}  
    [ -n "$CFGDBG" ] && echo read $path into line array with $m elements 

    ini=( ${ini[*]//\#*/} )                  # remove comments

    a=${#ini[*]}  
    [ -n "$CFGDBG" ] && echo completed replacements have $a elements
    ...
    p=${#ini[*]}  
    [ -n "$CFGDBG" ] && echo completed replacements have $p elements

    ini[0]=''                        # remove first element
    ini[$N + 1]='}'                  # add the last brace

    q=${#ini[*]}  
    r=${#ini[@]}  
    [ -n "$CFGDBG" ] && echo completed head and tail have $q elements $r
